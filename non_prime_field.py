...
class Polynomial:
    ...
    # Evaluation (by an integer or a PrimeField)
    def eval(self, y):
        if isinstance(y, int) or isinstance(y, PrimeField):
            z = 0
...
# Field of non-prime field defined using irreducible polynomial p. No checking of irreducibility
class NonPrimeField:
    def __init__(self, x, p):
        if isinstance(x, Polynomial) and isinstance(p, Polynomial) and x.l == p.l:
            _, self.x = x.divmod(p, True)
            self.p = p
            self.ord = x.l ** (len(p.coeffs) - 1) # order
            self.int = self.x.eval(x.l) # integer representation
        else:
            raise TypeError
